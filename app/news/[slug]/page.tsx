...
export default async function NewsDetailPage(props: PageProps) {
  ...
  return (
    <article className="min-h-screen bg-slate-50 pb-20 font-sans">
      ...
      {/* --- 1. HERO SECTION (Gambar Latar & Judul) --- */}
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 relative z-10 -mt-20">
        <div className="bg-white rounded-3xl p-8 sm:p-12 shadow-xl border border-slate-100">
          <ReactMarkdown 
            ...
            components={{
              p: ({node, ...props}) => (
                <p className="text-slate-700 text-lg leading-relaxed mb-8" {...props} />
              ),
              h1: ({node, ...props}) => (
                <h1 className="text-3xl font-bold text-slate-900 mt-12 mb-6" {...props} />
              ),
              h2: ({node, ...props}) => (
                <h2 className="text-2xl font-bold text-slate-800 mt-10 mb-4 border-b pb-2 border-slate-100" {...props} />
              ),
              h3: ({node, ...props}) => (
                <h3 className="text-xl font-bold text-blue-700 mt-8 mb-3" {...props} />
              ),
              ul: ({node, ...props}) => (
                <ul className="list-disc pl-6 mb-8 space-y-2 text-slate-700 marker:text-blue-500" {...props} />
              ),
              ol: ({node, ...props}) => (
                <ol className="list-decimal pl-6 mb-8 space-y-2 text-slate-700 marker:text-blue-500" {...props} />
              ),
              li: ({node, ...props}) => (
              ...
              blockquote: ({node, ...props}) => (
                <blockquote className="border-l-4 border-blue-500 bg-blue-50 py-4 px-6 italic text-slate-700 rounded-r-lg my-8" {...props} />
              ),
              a: ({node, ...props}) => (
                <a className="text-blue-600 font-semibold underline hover:text-blue-800 transition-colors" {...props} />
              ),
              img: ({node, ...props}) => (
          ...
          <div className="text-center">
             <p className="text-slate-400 text-sm mb-3">Bagikan artikel ini</p>
